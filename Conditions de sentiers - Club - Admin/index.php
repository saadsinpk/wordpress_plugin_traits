	...
	if(isset($_GET['editid'])) {
      ...
      echo '<link rel="stylesheet" href="https://ajax.googleapis.com/ajax/libs/jqueryui/1.8.24/themes/smoothness/jquery-ui.css" />';
	    ...
	    if(isset($_POST['update_multi_club'])) {
			...
			$sql = "SELECT * FROM Sentiers WHERE clubNo = '".$site_id."' ORDER BY ordre ASC";
			$result = $conn->query($sql);
			...
			<th scope="col">Action</th>
				...
				if ($result->num_rows > 0) {
				  ...
				  while($row = $result->fetch_assoc()) {
					...
					<td><a href='".get_site_url()."/wp-admin/admin.php?page=conditions_de_sentiers_club&editid=".$row['id']."'>Édition</a> | <a href='#' data-href='".get_site_url()."/wp-admin/admin.php?page=conditions_de_sentiers_club&deleteid=".$row['id']."' class='delete'>Supprimer</button></td></tr>";
				  }
				}
			echo "</table>";
...
function sidtechno_custom_fields(){
    ...
    if( 'site-info.php' == $pagenow OR 'site-new.php' == $pagenow ) {
        ...
        </tr><tr class="sidtechno_custom_fields">
